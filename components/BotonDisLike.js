...
export default function BotonDislike(props) {
    return (
        <Button onPress={() => { darLike(props.id, getId(), getDogId()) }}>DISLIKE!</Button>
    )
}
...
async function darLike(id, idUsuario, idPerro) {
    ...
    await axios.delete(`http://www.hotdogort.ml/api/likes/${idUsuario}/${idPerro}`, {
        usuarioId: `${usuario}`,
        idLike: `${id}`,
    })
